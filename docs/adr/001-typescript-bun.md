...
Use TypeScript 5.9 and Bun for v1 behind phase-zero evidence. The compiled binary, SQLite, MCP initialization, HTTP, Unix sockets, pinned A2A SDK, reviewed TCK allowlist, and emulated compiled app-server transport work locally. A real shared Codex connection, two-build Codex attestation, approval ownership, delivery reconciliation, and the two-agent demo remain release blockers.
