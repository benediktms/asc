...
| Crash or ambiguous runtime write   | Durable acceptance commits before success. Runtime requests are leased and fenced; a disconnect before write is deferred, while a disconnect after write enters `acceptance-unknown` and is never blindly retried.                                   | Real Codex history is not authoritative enough for automatic generic reconciliation; inconclusive cases require an operator. |
...
until the real-Codex attestation, shared-daemon, safe-delivery, reconciliation,
and approval-ownership gates documented by `acs codex doctor` are completed.
